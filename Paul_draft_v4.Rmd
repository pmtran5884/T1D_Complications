...
a2<-ggplot(Predict(DPNselect, Dr_Age, AvgA1c = c(5, 7, 9,11), fun = plogis))
c2<-ggplot(Predict(DPNselect, Dr_Age, avg.Systolic = c(120, 140, 160, 180), fun = plogis))

e2<-ggplot(Predict(ANselect, Dr_Age, AvgA1c = c(5, 7, 9,11), fun = plogis))
g2<-ggplot(Predict(ANselect, Dr_Age, avg.Systolic = c(120, 140, 160, 180), fun = plogis))
...
m2<-ggplot(Predict(DNselect, Dr_Age, AvgA1c = c(5, 7, 9,11), fun = plogis))
o2<-ggplot(Predict(DNselect, Dr_Age, avg.Systolic = c(120, 140, 160, 180), fun = plogis))


#save data
saveRDS(DPNselect,file = "/Users/paultran/Box/Students/T1D_complications/Shiny App/T1D_Complications/data/T1Dcomp_DPNmod_logit.rds")
saveRDS(ANselect,file = "/Users/paultran/Box/Students/T1D_complications/Shiny App/T1D_Complications/data/T1Dcomp_DANmod_logit.rds")
...
saveRDS(DNselect,file = "/Users/paultran/Box/Students/T1D_complications/Shiny App/T1D_Complications/data/T1Dcomp_DNEPmod_logit.rds")

#summary and plot
summary(DPNselect);summary(ANselect);summary(DRselect);summary(DNselect);plot(anova(DPNselect), what='proportion chisq');plot(anova(ANselect), what='proportion chisq');plot(anova(DRselect), what='proportion chisq');plot(anova(DNselect), what='proportion chisq');plot(Predict(DPNselect, fun=plogis));plot(Predict(ANselect, fun=plogis));plot(Predict(DRselect, fun=plogis));plot(Predict(DNselect, fun=plogis));plot(rms::calibrate(DPNselect, method="boot", B=500), las=1);plot(rms::calibrate(ANselect, method="boot", B=500), las=1);plot(rms::calibrate(DRselect, method="boot", B=500), las=1);plot(rms::calibrate(DNselect, method="boot", B=500), las=1); rms::validate(DPNselect, method="boot", B=500);rms::validate(ANselect, method="boot", B=500);rms::validate(DRselect, method="boot", B=500);rms::validate(DNselect, method="boot", B=500); cowplot::plot_grid(a,b,c,d,
                   e,f,g,h,
                   i,j,k,l,
                   m,n,o,p,
                   q,r,s,t,
...
```{r interaction stuff, didnt work}
##interaction##
# DPNint <- lrm(Peripheral_Neuropathy3 ~
#     Autonomic_Neuropathy3+
#     DM_Retinopathy3+
#     DM_Nephropathy3+
#     rcs(Dr_Age,3)+
...
#     rcs(AvgA1c,4),
#           data = T1Dcomp_alldata, x = TRUE, y = TRUE)
# print(DPNint, coef = FALSE)
# anova(DPNint)
# ANint <- lrm(Autonomic_Neuropathy3 ~
#     Peripheral_Neuropathy3+
#     DM_Retinopathy3+
#     DM_Nephropathy3+
#     rcs(Dr_Age,3)+
...
#     rcs(AvgA1c,4),
#           data = T1Dcomp_alldata, x = TRUE, y = TRUE)
# print(ANint, coef = FALSE)
# anova(ANint)
# ANint2 <- lrm(Autonomic_Neuropathy3 ~
#     Peripheral_Neuropathy3+
#     DM_Retinopathy3+
#     DM_Nephropathy3+
#     rcs(Dr_Age,3)+
...
```{r predict risk based on modifiable variables, didnt work}
#multiple values of any variable
a2<-ggplot(Predict(DPNselect, Dr_Age, AvgA1c = c(5, 7, 9,11), fun = plogis))
c2<-ggplot(Predict(DPNselect, Dr_Age, avg.Systolic = c(120, 140, 160, 180), fun = plogis))
